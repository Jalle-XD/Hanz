...
try:
	...
	print('[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mHanz X Code')
...
def login():
	try:
		...
		try:
			...
			li = '# KONEKSI TIDAK STABIL PERIKSA KONEKSI ANDA 📶 '
...
def menu(my_name,my_id):
	...
	cetak(panel(f'[bold white][[bold red]•[/][bold white]][/] [bold white]۝NAMA۝: [bold green]{my_name}[/]\n[bold white][[bold red]•[/][bold white]][/] [bold white]۝ID۝  : [bold green]{my_id}[/]\n[bold white][[bold red]•[/][bold white]][/] [bold white]۝IP۝  : [bold green]{ip}[/]',width=90,title=f"[bold green]INFORMASI",style=f"bold cyan"))
	cetak(panel(f'[bold white][[bold red]01[/][bold white]][/] [bold white] 𝙲𝚁𝙰𝙲𝙺 𝙿𝚄𝙱??𝙸𝙺 [[bold green] ON [bold white]][/]\n[bold white][[bold red]02[/][bold white]][/] [bold white] 𝙲𝚁𝙰𝙲𝙺  𝙼𝙰𝚂𝙰𝙻𝙻[[bold green] ON [bold white]][/]\n[bold white][[bold red]03[/][bold white]][/] [bold white]Cek Result [[bold green] ON [bold white]][/]\n[bold white][[bold red]00[/][bold white]][/] [bold red]Hapus Cookies [[bold green] ON [bold white]][/]',width=90,title=f"[bold green]LIST MENU",style=f"bold cyan"))
	...
	_____brayennn___xd____ = input(f' ╰─  𝐒𝐞𝐥𝐞𝐜𝐭 : ')
...
def dump_massal():
	...
	for met in range(jum):
		yz+=1
		kl = input(f' ╰─  {P}𝐌𝐚𝐬𝐮𝐤𝐚𝐧 𝐈𝐝𝐳 ??𝐚𝐧𝐠 𝐊𝐞 : '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			exit()
	try:
		print(f' ╰─  {P}𝐓𝐨𝐭𝐚𝐥 ??𝐝𝐳 𝐘𝐚𝐧𝐠 𝐓𝐞𝐫𝐤𝐮𝐦𝐩𝐮𝐥 : {x} : {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print(' ╰─  Unstable Signal ')
...
def setting():
	print('')
	cetak(panel(f'[bold white][[bold red]01[/][bold white]][/] [bold white] 𝙰𝙺𝚄𝙽 𝙾𝙻𝙳 [[bold green] Recommended[bold white]][/]\n[bold white][[bold red]02[/][bold white]][/] [bold white] 𝙰𝙺𝚄𝙽 𝙽𝙴𝚆  [[bold green]Very Recommended[bold white]][/]\n[bold white][[bold red]03[/][bold white]][/] [bold white] 𝙰𝙺𝚄𝙽 𝚁𝙰𝙽𝙳𝙾𝙼 [[bold green]Very Recommended[bold white]][/]',width=90,title=f"[bold green]SETTING URUTAN ID",style=f"bold cyan"))
	hu = input(f' ╰─ 𝐒𝐞𝐥𝐞𝐜𝐭 : ')
	if hu in ['1','01']:
...
[bold white][[bold red]03[bold white]] [bold white]Nama, Nama123, Nama1234, Nama12345 + Manual [[bold red]Not Recommended[bold white]]''',style='bold cyan',title='[bold green]SETTING PASSWORD',width=90))
	...
	if pwplus in ['03','3']:
		...
		pwpluss.append('no')
	cetak(panel(f'[bold white]               𝐓𝐚𝐦𝐛𝐚𝐡𝐤𝐚𝐧 𝐀𝐩𝐥𝐢𝐤??𝐬𝐢 (𝐑𝐞𝐜𝐨𝐦𝐦𝐞𝐧𝐝) [𝐘/𝐭] :  ',width=90,title=f"[bold green]APLIKASI",style=f"bold cyan"))
	_jembot_ = input(f' ╰─   𝐒𝐞𝐥𝐞𝐜𝐭 : ')
...
def passwrd():
	...
	print('')
	urut = []
	urut.append(panel(f'        [bold green]%s [bold white]'%(okc),width=90,title=f"[bold green]𝐇𝐀𝐒𝐈𝐋 𝐎𝐊 𝐓𝐄𝐑𝐒𝐈𝐌𝐏𝐀𝐍 𝐃𝐈 : ",style=f"bold cyan"))
	urut.append(panel(f'         [bold yellow]%s [bold white]'%(cpc),width=90,title=f"[bold yellow]𝐇𝐀𝐒𝐈𝐋 𝐂𝐏 𝐓𝐄𝐑𝐒𝐈𝐌𝐏𝐀𝐍 𝐃𝐈 :",style=f"bold cyan"))
	...
	print(f'{h} [𝐇𝐀𝐒𝐈𝐋 𝐎𝐊𝐓𝐄𝐑𝐒𝐈𝐌𝐏𝐀𝐍 𝐃𝐈 :] OK : %s '%(ok))
	print(f'{k}[𝐇𝐀𝐒𝐈𝐋 𝐂𝐏 𝐓𝐄𝐑𝐒𝐈𝐌𝐏𝐀𝐍 𝐃𝐈 :] CP : %s '%(cp))
...
if __name__=='__main__':
	...
	cetak(panel(f"""[bold white][[bold red]•[bold white]][bold cyan] > PERHATIAN ⚠️ :""",width=90,title=f'[bold green]WARNING ⚠️ :',style=f"bold cyan"))
	hanz_xy(f"\n\t{p}[{m}•{p}]{b} > Gunakan Scrip Dengan Sewajarnya\n\t{p}[{m}•{p}]{b} > Kalo error/Bug Bilang\n\t{p}[{m}•{p}]{b} > Author Tidak Bertangung Jawab Atas Apa Yang Terjadi\n\t{p}[{m}•{p}]{b} > Jika Terdapat Akun Checkpoin Simpan 1/2 Hari\n\t{p}[{m}•{p}]{b} > Jika Ip Sering Kena Spam Jangan Terlalu Paksa Crack\n\t{p}[{m}•{p}]{b} > Scrip Ini Boros Kuota\n\t{p}[{m}•{p}]{b} > Scrip Ini Blom Tentu Memuasakan Hasil {h}Hijau{x}\n\t{p}[{m}•{p}]{b} > Apa Bila Tidak Ada Hasil Mainkan Mode Pesawat 2/3 Detik\n\t{p}[{m}•{p}]{b} > Semoga Hari Mu Menyenangkan v:")
	time.sleep(1)
	cetak(panel(f"""[bold white][[bold red]•[bold white]][bold cyan] > TERIMAKASIH KEPADA ☣️ :  """,width=90,title=f"""[bold green]THANKS TO ☣️ :""",style=f"bold cyan"))
	hanz_xy(f'\n\t{p}[{m}•{p}]{b} > ALVINO ADIJAYA\n\t{p}[{m}•{p}]{b} > BRAYEN-XD\n\t{p}[{m}•{p}]{b} > DAPUNTA X\n\t{p}[{m}•{p}]{b} > BASARI-XD\n\t{p}[{m}•{p}]{b} > ACIL X CODE')
	time.sleep(1)
	cetak(panel(f"""[bold white][[bold red]•[bold white]][bold cyan] > DI PERBARUI OLEH ☣️  : """,width=90,title=f"""[bold green]UPDATE BY ☣️ :""",style=f"bold cyan"))
	hanz_xy(f'\n\t{p}[{m}•{p}]{b}> HANZ X CODE\n\t{p}[{m}•{p}]{b}> LIM-XD')
	time.sleep(1)
